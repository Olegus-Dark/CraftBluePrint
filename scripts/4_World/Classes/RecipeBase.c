	const int MAX_NUMBER_OF_INGREDIENTS_1 = 2;
	const int MAXIMUM_RESULTS_1 = 10;
	const float DEFAULT_SPAWN_DISTANCE_1 = 0.5;

modded class RecipeBase
{	
	
	string m_ItemsToCreate[MAXIMUM_RESULTS_1];
	ref array<string> m_Ingredients[MAX_NUMBER_OF_INGREDIENTS_1];
	
	ItemBase m_Items[MAX_NUMBER_OF_INGREDIENTS_1];
	
	ItemBase m_IngredientsSorted[MAX_NUMBER_OF_INGREDIENTS_1]; //if the recipe is valid, this array will contain all ingredients sorted against the recipe ingredients
	...
	float m_MinQuantityIngredient[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_MaxQuantityIngredient[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_MinDamageIngredient[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_MaxDamageIngredient[MAX_NUMBER_OF_INGREDIENTS_1];

	bool m_IngredientUseSoftSkills[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_IngredientAddHealth[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_IngredientAddQuantity[MAX_NUMBER_OF_INGREDIENTS_1];
	float m_IngredientSetHealth[MAX_NUMBER_OF_INGREDIENTS_1];
	bool m_IngredientDestroy[MAX_NUMBER_OF_INGREDIENTS_1];
	
	
	bool m_ResultSetFullQuantity[MAXIMUM_RESULTS_1];
	float m_ResultSetQuantity[MAXIMUM_RESULTS_1];
	float m_ResultSetHealth[MAXIMUM_RESULTS_1];
	float m_ResultSpawnDistance[MAXIMUM_RESULTS_1];
	int m_ResultToInventory[MAXIMUM_RESULTS_1];
	int m_ResultInheritsHealth[MAXIMUM_RESULTS_1];
	int m_ResultInheritsColor[MAXIMUM_RESULTS_1];
	int	m_ResultReplacesIngredient[MAXIMUM_RESULTS_1];
	bool m_ResultUseSoftSkills[MAXIMUM_RESULTS_1];
...
//	m_MinDamageIngredient = new float(10);
		for(int i = 0; i < MAX_NUMBER_OF_INGREDIENTS_1; i++)
		{
			m_Ingredients[i] = new array<string>;
			m_IngredientsSorted[i] = NULL;
		}
		
		for(i = 0; i < MAXIMUM_RESULTS_1; i++)
		{
			m_ResultSpawnDistance[i] = DEFAULT_SPAWN_DISTANCE_1;
		}
	...
	bool CheckIngredientMatch(ItemBase item1, ItemBase item2)
	{
		...
		bool found = false;
		for(int i = 0; i < MAX_NUMBER_OF_INGREDIENTS_1; i++)//all ingredients
		{
			found = false;
			array<string> tempArray = m_Ingredients[i];
			for(int x = 0; x < tempArray.Count(); x++)//particular ingredient array
			{
				for(int z = 0; z < MAX_NUMBER_OF_INGREDIENTS_1; z++)
				{
					if( m_Items[z] != NULL )
					{
	...
	void ApplyModificationsResults(ItemBase sorted[], array<ItemBase> results, ItemBase result, PlayerBase player)
	{
		...
		for(i = 0; i < m_NumberOfResults; i++)
		{
			...
			if( m_ResultInheritsHealth[i] != -1 )//<------m_ResultInheritsHealth
			{
				...
				{
					//float average_health = all_ingredients_health / MAX_NUMBER_OF_INGREDIENTS_1;
					float average_health01 = all_ingredients_health01 / MAX_NUMBER_OF_INGREDIENTS_1;
